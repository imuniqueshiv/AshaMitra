...
const UploadImage = ({ xrayImage, setXrayImage }) => {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      // --- NEW: GATEKEEPER ERROR HANDLER ---
      if (error.response && error.response.status === 400) {
        // Show the clean Gatekeeper message
        alert(`⚠️ ${error.response.data.message}`);
      } else {
        // Handle actual 500 server crashes like before
      ...
      clearInterval(progressInterval);
      // Reset the input so the user can try uploading a new file immediately
