...
export const analyzeXray = async (req, res) => {
  try {
    ...
    // ✅ HARDCODED PYTHON PATH (Updated to your working Anaconda GPU Environment)
    const pythonExecutable = "C:\\Users\\singh\\anaconda3\\envs\\ashamitra\\python.exe";
    ...
    pythonProcess.on("close", (code) => {
      ...
      try {
        ...
        if (result.error === "INVALID_IMAGE") {
          console.warn("⚠️ Invalid Image Detected by Gatekeeper");
          return res.status(400).json({
            success: false,
            message: result.message // This sends the Python Gatekeeper message to frontend
          });
        }
        // -----------------------------

        // Generic fallback for actual Python crashes
        if (result.error) {
